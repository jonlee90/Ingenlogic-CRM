...
<div class="overlay-form overlay-lead-product">
  ...
  <div class="lead-list-available">
    <table id="tbl-lead-product-available">
      <thead><tr> <th></th> <th>Services</th> <th>Name</th> <th>Spiff</th> <th>Residual</th> <th>Price</th> </tr></thead>
      <tbody>
  ...
  {!! Form::open(['url'=> route('master.lead.ajax-quote-mrc', ['quote_id'=> enc_id($quote->id)]), 'class'=>'frm-mrc']) !!}
    ...
    <table class="tbl-lead-prod-list mrc">
      ...
      @forelse ($quote->mrc_prods as $prod)
        ...
        <tr>
          ...
          <td>{!! Form::number('spiff[]', $prod->spiff_rate, ['step'=> 0.01, 'required']) !!}<label>%</label></td>
          <td>{!! Form::number('resid[]', $prod->residual_rate, ['step'=> 0.01, 'required']) !!}<label>%</label></td>
          <td>
            <label class="fa-usd"></label>
            {!! Form::number('price[]', $prod->price, ['step'=> 0.01, 'required']) !!}
          </td>
          <td>{!! Form::number('qty[]', $prod->qty, ['step'=> 1, 'required',]) !!}</td>
          <td>
            <label class="fa-usd"></label>
            <span class="subtotal">{{ number_format($prod->price * $prod->qty, 2, '.','') }}</span>
          </td>
        </tr>
        @else
        <tr class="grayed">
          ...
          <td><label class="fa-usd"></label> {{ $prod->price }} </td>
          <td>{{ $prod->qty }}</td>
  ...
  {!! Form::open(['url'=> route('master.lead.ajax-quote-nrc', ['quote_id'=> enc_id($quote->id)]), 'class'=>'frm-nrc']) !!}
  ...
  <table class="tbl-lead-row-src">
    <tr>
      ...
      <td>{!! Form::number('spiff[]', 0, ['step'=> 0.01, 'required']) !!}<label>%</label></td>
      <td>{!! Form::number('resid[]', 0, ['step'=> 0.01, 'required']) !!}<label>%</label></td>
      <td>
        <label class="fa-usd"></label>
