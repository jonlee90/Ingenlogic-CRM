...
trait LeadTrait
{
  ...
  */
  ...
  public function traitAlertMod(Request $request, $frm_url)
  {
    $log_src = $this->log_src.'@traitAlertMod';
    $preapp = $request->get('preapp');
    $lead_logs_id = $request['log'];
    ...
    $follower_count = count($request->users);
    
    $followers = DB::table('login_users')->whereIn('id', $request->users)->get();
  
    $alerted_followers = DB::table('lead_alert')->where('lead_id', $lead_id)
                                                ->where('lead_logs_id', $lead_logs_id)->get();

    $alerted_followers = DB::select(
          " SELECT a.login_user_id as user_id, IF(u.id >0 AND a.id >0, 1,0) AS valid,
          IF(u.id >0 AND a.id >0, TRIM(CONCAT(u.fname,' ',u.lname)), f.name) AS f_name,
          IF(u.id >0 AND a.id >0, u.title, f.title) AS title,
          IF(u.id >0 AND a.id >0, u.tel, f.tel) AS tel,
          IF(u.id >0 AND a.id >0, u.email, f.email) AS email
          FROM lead_alert as a
          LEFT JOIN login_users as u ON u.id = a.login_user_id
          LEFT JOIN lead_follower_agents as f ON f.user_id = a.login_user_id
          WHERE a.lead_id =:lead_id AND a.lead_logs_id =:lead_logs_id
        ", [$lead_id, $lead_logs_id]);
  
		$html_output =
        view('leads.form-alert')
          ->with('lead_id', $lead_id)
          ->with('frm_url', $frm_url)
          ->with('followers', $followers)
          ->with('lead_logs_id', $lead_logs_id)
          ->with('alerted_followers', $alerted_followers)
          ->render().'
          
      <script>aoLeadAlert()</script>
		';    
		return json_encode([
			'success'=>1, 'error'=>0,
			'html'=> $html_output
  ...
  public function traitAlertSend (Request $request)
  {
    $log_src = $this->log_src.'@traitAlertSend';
    $preapp = $request->get('preapp');
    $agency_id = dec_id($preapp->agency_id);
    $lead_logs_id = $request['lead_logs_id'];
    ...
    $alerted_followers = DB::select(
      " SELECT login_user_id  FROM lead_alert  WHERE lead_logs_id =:lead_log_id AND is_read = 1  
    ", [$lead_id]);
    ...
    for ($i =0; $i < $user_count; $i++) {
      $r_user_id = dec_id($user_id[$i]);

      $db_insert_users[] = [
        'lead_logs_id'=> $lead_logs_id,
        'login_user_id'=> $r_user_id,
        'is_read'=> 0,
        'lead_id'=> $lead_id
      ];
    }

    DB::table('lead_alert')->insert($db_insert_users);
    

    log_write('Lead x Alert Sent.', ['Lead Id'=> $lead_id, 'Lead Log Id'=> $lead_logs_id, 'How many sent' => $user_count ]);
    return $this->jsonReload($lead_id, $agency_id);
  }
